...
export function SplitIntro() {
  return (
    <section
      id="top"
      className="relative min-h-[calc(100vh-80px)] px-6 py-12 max-w-4xl mx-auto flex items-center"
    >
      <Reveal delay={80} from="up" className="w-full">
        <div className="relative glass-card no-glow rounded-3xl p-6 sm:p-8 flex flex-col overflow-hidden">
          ...
          <div className="relative pt-6 mt-6 border-t border-border flex flex-wrap gap-3">
            <a
              href="#projects"
              className="no-glow rounded-full bg-primary text-primary-foreground font-mono text-sm px-5 py-2.5 hover:opacity-90 transition"
            >
              ./view_projects →
            </a>
            <a
              href="#contact"
              className="no-glow rounded-full bg-card border border-border font-mono text-sm px-5 py-2.5 hover:bg-accent transition"
            >
              ./contact_me
            </a>
          </div>
