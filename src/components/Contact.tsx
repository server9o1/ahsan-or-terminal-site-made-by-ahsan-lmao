...
function SocialLink({ name, url }: { name: string; url: string }) {
  ...
  return (
    <a
      ...
      className="relative flex items-center justify-between rounded-full bg-card/60 border border-border px-5 py-3 font-mono text-sm hover:bg-accent hover:translate-x-1 transition-all overflow-hidden"
    >
...
function EmailRow() {
  ...
  return (
    <div className="group mt-10 inline-flex items-center gap-2 rounded-full bg-primary text-primary-foreground font-mono text-sm pl-5 pr-2 py-2">
      <a href={`mailto:${EMAIL}`} className="hover:opacity-90 transition">
        {EMAIL} ↗
      </a>
      <button
        onClick={copy}
...
export function Contact() {
  return (
    <section id="contact" className="relative z-10 px-6 py-24 max-w-6xl mx-auto">
      <Reveal>
        <div className="glass-card rounded-3xl p-8 sm:p-14 grid md:grid-cols-2 gap-10 items-center">
          <div>
